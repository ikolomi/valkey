...
def run_loaders(loader_specs, work_dir, fifo_name="start.fifo",
    ...
    """Spawn all loaders behind a FIFO start-barrier, release them simultaneously,
    wait for self-exit (duration-bounded), and collect per-loader artifacts.

    Returns a list of ``{command, index, returncode, stdout_path, stderr_path,
    achieved_rps}``.
    """
    ...
    spawned = []  # (spec, proc, out_path, err_path, out_fh, err_fh)
    max_dur = 0
    for spec in loader_specs:
        ...
        # sh wrapper: block on the FIFO, then exec the loader (stdout/stderr via Popen)
        cmd = ["sh", "-c", 'read _ < "$1"; shift; exec "$@"', "sh", fifo_path] + spec["argv"]
        proc = subprocess.Popen(cmd, stdout=out_fh, stderr=err_fh)
        spawned.append((spec, proc, out_path, err_path, out_fh, err_fh))
        a = spec["argv"]
        if "--duration" in a:
            max_dur = max(max_dur, int(a[a.index("--duration") + 1]))
    ...
    results = []
    wait_timeout = max_dur + timeout_slack
    for spec, proc, out_path, err_path, out_fh, err_fh in spawned:
        try:
