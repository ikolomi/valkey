...
TEST_F(CompressionRegistryTest, AddWithPromoteMakesActive) {
    uint32_t id = compressionRegistryAdd(fakeDictBytes(100), 100, 1);
    ASSERT_NE(id, (uint32_t)COMPRESSION_DICT_ID_NONE);

    compressionDictPair *active = compressionDictGetActive();
...
TEST_F(CompressionRegistryTest, AddWithoutPromoteIsRetiring) {
    uint32_t id = compressionRegistryAdd(fakeDictBytes(100), 100, 0);
    ASSERT_NE(id, (uint32_t)COMPRESSION_DICT_ID_NONE);
    ...
    compressionDictPair *d = compressionDictLookup(id);
    ASSERT_NE(d, nullptr);
    ASSERT_EQ(d->state, DICT_STATE_RETIRING);
}

TEST_F(CompressionRegistryTest, SecondPromoteRetiresFirst) {
    uint32_t id1 = compressionRegistryAdd(fakeDictBytes(100), 100, 1);
    uint32_t id2 = compressionRegistryAdd(fakeDictBytes(100), 100, 1);
    ...
    compressionDictPair *d1 = compressionDictLookup(id1);
...
TEST_F(CompressionRegistryTest, StartRetirementSnapshotsWorkerGens) {
    ...
    compressionRegistryAdd(fakeDictBytes(100), 100, 1);
...
TEST_F(CompressionRegistryTest, CanFreeWithMultipleWorkersOneBehind) {
    ...
    uint32_t id = compressionRegistryAdd(fakeDictBytes(100), 100, 1);
    compressionRegistryAdd(fakeDictBytes(100), 100, 1); /* retire first */
...
TEST_F(CompressionRegistryTest, TryGcFreesSafeDicts) {
    uint32_t id1 = compressionRegistryAdd(fakeDictBytes(100), 100, 1);
    compressionRegistryAdd(fakeDictBytes(100), 100, 1); /* retires id1 */
...
static void countCallback(const compressionDictPair *d, void *ctx) {
    (void)d;
    int *count = (int *)ctx;
    (*count)++;
}
...
TEST_F(CompressionRegistryTest, ForEachIteratesAllNonRetired) {
    compressionRegistryAdd(fakeDictBytes(100), 100, 1);
    compressionRegistryAdd(fakeDictBytes(100), 100, 0);
    compressionRegistryAdd(fakeDictBytes(100), 100, 1);
