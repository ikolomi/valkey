...
void compressionWorkersStop(void) {
     ...
     *
     * Without the decRef here, ASan-instrumented multi-test gtest
     * binaries flag a leak whenever a test exits with in-flight jobs
     * still in the inbox. Production single-process shutdown was
     * tolerant of the leak; the gtest binary is not. */
    void *leftover;
