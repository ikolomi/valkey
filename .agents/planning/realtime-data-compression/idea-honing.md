...
> **Status note (2026-06-14):** This document is preserved as the historical Q&A audit trail of v1 design decisions. **`design/detailed-design.md` is the source of truth for the current design.** Two areas of `idea-honing.md` were materially redesigned post-walkthrough:
>
> - **Q5 (master switch on/off model)** was reworked into the declarative 3-state model (`compression-master-switch: compression | decompression | off`) with a separate sweeper switch (`compression-automatic-sweeper: enabled | disabled`) and a manual force command (`COMPRESSION SWEEP FORCE`). See `detailed-design.md` §2.1 (R2.1.1 – R2.1.7) for the current model. The Q5 answer below describes the original `compression-enabled yes/no` bool model and is **superseded**.
...
> - **Q15 (testing strategy — §7.1 transparency-mode harness)** describes a `--compression` flag whose server-config payload uses the old `--compression-enabled yes` etc. The flag's design intent is still valid, but the specific config names are now superseded — see `detailed-design.md` §7.1 for the current set (`--compression-master-switch compression --compression-automatic-sweeper enabled` plus the eligibility-relaxing knobs).
>
...
- **Skip hot items — policy-aware.** Valkey's 24-bit `robj->lru` field stores different metrics depending on `maxmemory-policy` (see `src/lrulfu.h`). The eligibility predicate uses the appropriate metric for each:
  - **LRU / `noeviction` modes** (`robj->lru` is seconds-based): skip if `lru_idle_secs(obj) < compression-min-idle-seconds` (default `60 s`). `robj->lru` is touched on every read AND write (gated only by `LOOKUP_NOTOUCH` and fork), so this single threshold gates eligibility on the "value has been quiet long enough to be worth compressing" property — covering both write-recency and read-recency with one signal.

      _Earlier drafts of this doc exposed two knobs in this branch — `compression-settle-seconds` ("recent-write protection") and `compression-min-idle-seconds` ("recent-access protection"). Both compared to the same `lru_idle_secs(obj)` metric, so the effective behavior was always `idle >= max(settle, min_idle)` — the second knob added no expressive power v1 could deliver, only a footgun (operators tuning them differently expecting different effects). Per YAGNI and Valkey's preference for minimal operator surfaces (PR #1 Thread #3, the 5+11 → 5+10 split), v1 ships only the single knob. v2 can reintroduce a write-time-specific knob non-breakingly when per-object write-time tracking lands._
  - **LFU mode** (`robj->lru` encodes a freq counter, no per-second timestamp): apply the freq threshold instead — skip if `lfu_freq(obj) >= compression-lfu-threshold` (default `5`, on the 0–255 log scale). The time-based knob is inactive in this mode; the freq counter IS the access-recency signal.
- **Skip post-compression if we don't actually save.** After the worker compresses, on the main thread we compare `compressed_size + header >= uncompressed_size * (1 - compression-min-savings-ratio)` (default `10%`). If true: discard the compressed form, leave the value uncompressed, and increment `compression_skipped_incompressible`. v1 does **not** track per-key rejection state. Under a fixed dict and a fixed workload, the probability that any given write at key K produces compressible bytes is constant — time-based throttling between retries doesn't shift this probability, it only spaces attempts. CPU bounding is already provided at the sweep level (`compression-sweep-max-cpu-pct`); per-key throttling would be redundant and would add correctness burden (stale entries on overwrite or in-place mutation). The "rejection might now compress" trigger is **dict change**, which is handled at the system level: rejections contribute their actual measured ratio to `compression_live_ratio_10m` — typically in `[0.9, 1.05]` since the net-savings guard rejected them for being too close to 1.0 — so sustained high rejection rate naturally trips the drift threshold and triggers retraining (R2.3.5).
...
- **(Q6a)** *Superseded by review (see PR threads T-3167775681 / T-3167859080), then refined again during S2.2 implementation review: the eligibility model is **policy-aware**, not policy-uniform.* In **LRU and `noeviction` modes**, a single threshold (`compression-min-idle-seconds`) applies to `lru_idle_secs(obj)` — Valkey's 24-bit `robj->lru` field is seconds-based in these modes and is touched on every read AND write, so this single signal covers both write-recency and read-recency intent. In **LFU mode**, `robj->lru` encodes a freq counter (not a timestamp), so the time-based threshold does not apply meaningfully; the LFU branch (`lfu_freq(obj) < compression-lfu-threshold`) filters by frequency instead. (See `src/lrulfu.h` for the encoding details. The earlier dual-knob surface — `compression-settle-seconds` + `compression-min-idle-seconds` — was dropped per YAGNI: both compared to the same metric in v1, so the second knob added no expressive power, only a footgun. v2 may reintroduce a write-time-specific knob non-breakingly when per-object write-time tracking lands.)
- **(Q6b)** v1 does **not** include a per-key retry guard. *Superseded by review during S2.3 implementation (PR #10): the originally proposed dict-scoped side hashtable was dropped before merge.* Under a fixed dict and a fixed workload, the probability that any given write at key K produces compressible bytes is constant — time-based throttling between retries doesn't shift this probability, only spaces attempts. CPU bounding is already provided at the sweep level (`compression-sweep-max-cpu-pct`); per-key throttling would be redundant with that, while adding correctness burden (every mutating code path — `dbOverwrite`, APPEND, SETRANGE, BITOP, BITFIELD, module DMA writes — would need a `Clear` integration point to avoid suppressing compression of legitimately-changed values). The right trigger for "the rejection might now compress" is **dict change**, which is handled at the system level: rejected attempts contribute their actual measured ratio to `compression_live_ratio_10m`, so a sustained high rejection rate naturally trips the drift threshold and triggers retraining (R2.3.5). After dict promotion, every previously-rejected eligible key is naturally retried on the next sweep tick. Counter tracked in `INFO compression` (`compression_skipped_incompressible`).
