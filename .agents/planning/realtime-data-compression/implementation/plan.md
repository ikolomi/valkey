...
- [x] **S2.9 — Master switch + sweep** (PR #26): sweep state machine (`src/compression_sweep.{c,h}`) mirrors @GilboaAWS's `compressionTrainCron` pattern — monotime-budgeted scan spliced across cron ticks, persistent `(current_db, cursor)` cursor, single-flight (overlapping requests dropped). Per-tick budget computed from `compression-sweep-max-cpu-pct` × `server.hz` (default 25% × hz=10 → 25 ms/tick, 10 µs floor). `compressionSweepRequest(direction)` is the internal entry point; `COMPRESSION SWEEP [direction=compress|decompress]` is the operator-facing command (replaces the Phase 0 stub in `compressionSweep` + adds new `src/commands/compression-sweep.json`). Runtime master-switch transition `compression-enabled no → yes` auto-triggers a compress-direction sweep via a new `applyCompressionEnabled` apply hook on the bool config (R2.1.3); `yes → no` does NOT auto-trigger decompress (R2.1.4 — operator must explicitly run `direction=decompress`). Sweep callback calls `compressionEnqueueCandidate` (compress) or `compressionPermanentlyDecompress` (decompress) per key; neither path calls `signalModifiedKey` (R2.9.2). `server.db[j]` NULL-guard mirrors PR #25's fix in `compression_train.c`.
- [x] **S2.10 — Cron integration**: `compressionCron` wired into `serverCron` by PR #20 (training); PR #26 added the `compressionSweepCron()` call after `compressionTrainCron()` inside the same function. Sweep pacing (`compression-sweep-max-cpu-pct`) lands with PR #26. Dict drift-ratio evaluation is a placeholder hook in `compressionSweepCron` and gets implemented by @GilboaAWS in S1.4.
