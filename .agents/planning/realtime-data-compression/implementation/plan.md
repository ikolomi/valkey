...
- [ ] **Topic-2 PR-B — `compression-stress.tcl`**: integration stress test exercising the full hot path (master=compression + sweeper=enabled + write/read mixed workload over runtime-generated dict). USES the helpers from Topic-2 PR-A. First end-to-end confidence on the merged S2.x stack against a real workload.
- [ ] **Topic-1 — maxmemory ↔ compression interaction (docs only)**: design-doc PR adding R2.8.4 "savings are workload-dependent" prose + R2.5.7 cross-reference noting permanent-decompress is sticky until eviction/sweeper. No code change; existing INFO surface (used_memory, compression_compressed_objects, compression_total_*_bytes, evicted_keys) is sufficient for operator diagnosis. Lands after Topic-2 PR-B so we can revisit whether any new metric is justified by the stress test's actual peak observations.
