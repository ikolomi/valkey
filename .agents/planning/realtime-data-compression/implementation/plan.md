...
- [ ] **S2.9 — Master switch + sweeper (declarative model)**: implements the new declarative master-switch + active-sweeper surface per design §2.1 (R2.1.1–R2.1.7) — `compression-master-switch` enum (`compression`/`decompression`/`off`), `compression-active-sweeper` enum (`enabled`/`disabled`), `compression-active-sweeper-interval` (default 0 = single-pass-on-direction-change-or-force). Apply hooks for both transitions including auto-retire of the active dict on transitions INTO `decompression` (R2.1.5). Replaces the prior bool `compression-enabled` and prior PR #26 sweep state machine.
