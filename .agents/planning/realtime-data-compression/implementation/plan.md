...
- [x] **S1.2 — Training sampler (main thread)**: kvstore shard iteration + contiguous-buffer sample copy, spliced across `serverCron` ticks. Implements R2.3.6 corrected flow (per Thread #29). `LOOKUP_NOTOUCH` semantics. **Merged in PR #20.** Follow-up fix in PR #25 (NULL-guard for sparse `server.db[]` iteration).
- [x] **S1.3 — Bio train job (`BIO_COMPRESSION_TRAIN`)**: accepts `(buffer, sizes[], count)`, calls `ZDICT_trainFromBuffer`, creates CDict/DDict on the bio thread, signals completion via an atomic result pointer. Never touches `robj`/`kvstore`/refcounts. **Merged in PR #20.**
- [x] **S1.4 — Train completion + promotion on main thread**: cron polls the atomic result, promotes via `compressionRegistryAdd`. CDict/DDict creation moved to the bio thread (keeps the cost off the main thread). **Merged in PR #20.** The R2.3.5 drift-retrain trigger is a placeholder pending the `compression_live_ratio_10m` metric from S2.5+ worker results; drift-detection wiring is tracked under **S1.5** (Phase 2).
