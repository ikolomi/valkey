...
- **S2.9 — Master switch + sweeper (declarative model)**: implements design §2.1 (R2.1.1–R2.1.7). Replaces the prior bool `compression-enabled` and the prior PR #26 sweep approach. Split into 3 PRs:
  - [x] **C1 (PR #29 merged)** — Config plumbing: `compression-master-switch` enum (`compression`/`decompression`/`off`), `compression-active-sweeper` enum (`enabled`/`disabled`), `compression-active-sweeper-interval`. Apply hooks for master + sweeper, including auto-retire of the active dict on transitions INTO `decompression` (R2.1.5). `compression-master-switch=compression` gating in `compressionIsEligible` and `compressionEnqueueModified`.
  ...
  - [x] **C3 (PR #31)** — Sweeper engine + `COMPRESSION SWEEP FORCE`. State machine (DISABLED ↔ IDLE ↔ SCANNING ↔ SLEEPING) driven by `compressionCron`. Sub-tick edge fix via `compressionSweepNotifyMasterSwitchChanged()` (apply-hook → cron) so transitions like `compression→off→compression` faster than a cron poll aren't missed. Per-key dispatch by master-switch direction (compress→workers, decompress→main thread). Non-functional-state warning for `master=compression + threads=0`. 15 new gtest cases (382 total) + 9 new Tcl tests (22 total).
...
**End of Phase 1 gate:**
- Feature works end-to-end on a single replica set with `compression-master-switch compression` + `compression-active-sweeper enabled` + manual dict training.
