...
- [x] **S2.14 — kvstore-direct reader audit (DEBUG DIGEST crash)**: the follow-up to the S2.13 discovery. Audited every `getDecodedObject` caller + kvstore-direct iterator. **Only `DEBUG DIGEST` / `DEBUG DIGEST-VALUE` crash** (via `xorObjectDigest`'s `OBJ_STRING` branch → `mixStringObjectDigest`/`xorStringObjectDigest` → `getDecodedObject`); both deliberately bypass `lookupKey` (digest must work on logically-expired keys). Fix: route the two digest helpers through `objectGetUncompressedView` (R2.5.2), mirroring PR #24's `rdbSaveStringObject`/`rioWriteBulkObject` pattern. Audited-safe: `OBJECT *` (LOOKUP_NO_BYTES → strEncoding "compressed"), `DEBUG SDSLEN` (guarded by `!sdsEncodedObject`), `DEBUG OBJECT` (strEncoding + PR#24-fixed `rdbSavedObjectLen`), all argv/already-lookupKey'd `getDecodedObject` callers. Test `tests/unit/compression-debug-readers.tcl` (`external:skip`): DIGEST/DIGEST-VALUE don't crash + **digest is byte-identical compressed vs decompressed** (transparency) + DEBUG RELOAD round-trip. Design R2.5.7 updated to list the digest readers among the explicit-decompression sites. **Deferred (not crashes):** `MEMORY USAGE` reports uncompressed size (decompresses via transient view) = S3.5 @GilboaAWS; `DEBUG OBJECT` missing R2.7.2 dictID/compressedlength fields = feature gap. *(PR #43)*
