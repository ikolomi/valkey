...
- **R2.1.3** `no → yes` transition: master switch turns on. The write-path eligibility predicate (R2.2) starts returning true for qualifying values, and `signalModifiedKey` routes them to the background-compression candidate path (R2.4). When an active dict exists in the registry, the worker pool processes those candidates and installs compressed frames back into the kvstore. When no active dict exists yet (R2.1.5 third state), the candidate path no-ops at the active-dict guard inside `compressionEnqueueCandidate` — nothing is queued and the value stays RAW in the kvstore. Training is independent of the master-switch transition (its own R2.3.5 triggers fire on DB total-keys count threshold, drift, or manual `COMPRESSION TRAIN`); once a dict is eventually promoted, subsequent writes to any eligible key get compressed normally — but values that were written while no dict was active remain RAW unless they are written again. **Existing values in memory at toggle time are not retroactively compressed.** Operator explicitly runs `COMPRESSION SWEEP direction=compress` to enqueue every eligible existing value (RAW pre-existing values, RAW values written while no dict was active, and any RAW value an operator created via the in-place permanent-decompress paths) for compression in the background. (Q5)
