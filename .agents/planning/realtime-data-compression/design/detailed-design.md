...
- **R2.1.5** Master-switch transitions:
  - **Any direction change to a productive direction (`compression` or `decompression`)** with either (a) `compression-automatic-sweeper=enabled` OR (b) a scan currently in flight (force-pass or automatic) → reset cursor and schedule a fresh pass on the next cron tick. The current pass — if any — is aborted and restarted from the top with the new direction; otherwise it would yield half-old-direction / half-new-direction work, inconsistent with the operator's declared state. With `compression-automatic-sweeper=disabled` AND no scan in flight, direction change is just configuration; no pass is scheduled until the operator runs `COMPRESSION SWEEP FORCE`.
  - **Any direction change to `off`** → abort any in-flight scan (cursor cleared, no replacement scheduled). The sweeper has no direction; the next cron tick is a no-op.
